...
struct Coordinates {
        public:
        // CONSTRUCTOR
                Coordinates(const coord_t x = 0., const coord_t y = 0.) {
                        this->setCoord(x, y);
                }
        // PUBLIC METHODS
                ...
                float distanceTo(const Coordinates& othr) const {
                        return sqrt((this->_x - othr._x)*(this->_x - othr._x)+(this->_y - othr._y)*(this->_y - othr._y));
                }
        // ACCESSORS
                coord_t x() const { return this->_x; }
                coord_t y() const { return this->_y; }
                void setX(coord_t x) { this->_x = x; }
                void setY(coord_t y) { this->_y = y; }
                void setCoord(coord_t x, coord_t y) { this->setX(x); this->setY(y); }
        // OPERATORS
...
class Vertex : public Coordinates {
        public:
        // CONSTRUCTOR
                Vertex(const coord_t = 0., const coord_t = 0., Edge* = NULL);
